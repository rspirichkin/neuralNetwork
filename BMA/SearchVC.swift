...
class SearchViewController: UIViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        if collectionView.tag == 100 {
            ...
            newCell.textField.text = (indexPath.section * n + indexPath.row+1).description
    ...
    @IBAction func actionSearch(sender: AnyObject) {
        ...
        for j in 0..<h {
            for i in 0..<n {
                ...
                signals.append(Node(Uin: S1r, Uout: S1r))
            }
        }
        let signalsOut = machineDetermination(&model, signals)
        for j in 0..<h {
            for i in 0..<k {
                (collectionViewM2.cellForItemAtIndexPath(NSIndexPath(forRow: i, inSection: j)) as! SignalCollectionViewCell).textField.text = signalsOut[j*k + i].Uin.format(".0")
            }
        }
        //collectionViewM1.backgroundColor = UIColor.redColor()
    }
